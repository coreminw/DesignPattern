...
public class CalcGUIV1 extends JFrame implements ActionListener {
    ...
    CmdButton[] buttons = new CmdButton[buttonText.length];
    ...
    public JPanel getButtonPanel() {
        ...
        for (int i = 0; i < 9; i++) {
            buttons[i] = new CmdButton1(calculator, display);
        ...
        for (int i = 10; i <14 ; i++) {
            buttons[i] = new CmdButton3(calculator);
            buttons[i].setText(buttonText[i]);
            buttons[i].setPreferredSize(buttonDimension);
            buttons[i].addActionListener(this);
            buttonPanel.add(buttons[i]);
        }
        buttons[14] = new CmdButton2(calculator, display);
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getSource() instanceof CmdButton){
            CmdButton cmdButton = (CmdButton) e.getSource();
            cmdButton.execute();
        }
